...
function generateTestData() {
    ...
    var url = "http://qa4flights.flydubai.com/en/results/ow/a1c1i1/DXB_SKT/20180117";
    ...
    if(!bookingPage.panel.panel.Exists)
    {
      aqUtils.Delay(20000);
    }   
    ...
    for (i = 0; i < adults; i++) {
      ...
      fakerObject = (i === 0) ? getAdultData("primary") : getAdultData("secondary");
        ...
        paxForm.Panel(0).FindChild("idStr", "pax" + count + "c1", 10).Select("atitleSelect" + i).ClickItem(fakerObject.Title);
        paxForm.Panel(0).FindChild("idStr", "pax" + count + "c2", 10).SetText(fakerObject.FirstName);
        paxForm.Panel(0).FindChild("idStr", "pax" + count + "c4", 10).SetText(fakerObject.LastName);

        if (i === 0) {
            paxForm.Panel(0).FindChild("idStr", "pax1c5", 10).Keys(fakerObject.EmailAddress);
    ...
    var chkTermsAgree = paymentSection.panel2.panel.panel.checkboxTermsAgree;
    if (chkTermsAgree.WaitProperty("Enabled", true, 5000))
        chkTermsAgree.ClickChecked(true);
        
    var btnPayNow = paymentPage.formPaymentcompform.panel.panel.panel.panel.panel2.buttonPayNow;
    ...
    writeToFile(JSON.stringify(bookingObject), "D:\\TestData\\TestData_A-001.txt");
}
